...
export const FeaturedCard = ({ item, onPress } : CardsProps) => {
  return (
    <TouchableOpacity onPress={onPress} className={"flex flex-col items-start w-60 h-80 relative"}>
      ...
      <View className="flex flex-row items-center bg-white/90 px-3 py-1.5 rounded-full absolute top-5 right-5">
        ...
        <Text className={"text-xs font-rubik-bold text-primary-300 ml-1 mt-1"}>{item.rating}</Text>
      </View>
...
export const Card = ({ item, onPress } : CardsProps) => {
  return (
    <TouchableOpacity onPress={onPress} className={"flex-1 w-full px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-300 relative mt-5"}>
      <View className="flex flex-row items-center absolute top-5 right-5 bg-white/90 px-2 py-1 rounded-full z-50">
        <Image source={icons.star} className={"size-2.5"} />
        <Text className={"text-xs font-rubik-bold text-primary-300 ml-0.5 mt-1"}>{item.rating}</Text>
      </View>
